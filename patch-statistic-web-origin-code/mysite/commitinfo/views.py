...
def zfrobisher_page(request):
    tablename='Statistics of zFrobisher Project'
    if 'year' in request.GET:
	year=request.GET.get('year')
	request.session['zfrobisher_year']=year
    else:
	if 'zfrobisher_year' in request.session:
	    year=request.session['zfrobisher_year']
	else:
	    year='all time'
    
    return render_to_response('info_table.html',show_table(year,zFrobisher_Project,tablename,request))
